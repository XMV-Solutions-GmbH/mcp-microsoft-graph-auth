...
- **Licensing (overrides `ENGINEERING_PRINCIPLES.md` § 11).** This repo is **OSS**, not XMV-proprietary. It is **dual-licensed `MIT OR Apache-2.0`** at the user's option — see [`LICENSE-MIT`](LICENSE-MIT) and [`LICENSE-APACHE`](LICENSE-APACHE). There is **no single `LICENSE` file** and **no XMV Proprietary Licence** here. Every source file's SPDX header uses `SPDX-License-Identifier: MIT OR Apache-2.0`, copyright `XMV Solutions GmbH`, and the human author as `SPDX-FileContributor`. Both `LICENSE-MIT` and `LICENSE-APACHE` are exempt from headers (they *are* the licences). The § 11 body text that refers to `LicenseRef-XMV-Proprietary` and a single `LICENSE` file does **not** apply to this repo.
